...
export class BMOSavingsImporter extends BaseImporter {
  name = 'BMO Savings';
  description = 'CSV format';
  supportedFormats = ['BMO Savings CSV'];
  
  detectFormat(headers: string[]): boolean {
    // BMO Savings CSV has headers: First Bank Card,Transaction Type,Date Posted, Transaction Amount,Description
    ...
    console.log('BMO Savings format detection - clean headers:', cleanHeaders);
    ...
    console.log('BMO Savings format detection:', {
    ...
    // Must have all required headers for BMO Savings format
  ...
  parseRow(row: string[], mapping: ColumnMapping, accountType?: string): Partial<Transaction> | null {
    try {
      ...
      if (!date) {
        console.warn(`BMO Savings: Invalid date format: ${dateStr}`);
        return null;
      }
      ...
      if (rawAmount === 0) {
        console.warn(`BMO Savings: Invalid amount: ${amountStr}`);
        return null;
      }
      ...
      console.error('BMO Savings: Error parsing row:', error, row);
      return null;
    }
  }
  ...
  private cleanPayeeName(payeeStr: string): string {
    // Remove BMO Savings-specific prefixes and clean up the payee name
    return payeeStr
      .replace(/^\[DS\]/i, '') // Remove [DS] prefix
      .replace(/^\[CW\]/i, '') // Remove [CW] prefix
      .replace(/^\[IN\]/i, '') // Remove [IN] prefix (interest)
      .replace(/^\[M\/C\]/i, '') // Remove [M/C] prefix
      .replace(/^PROMOTIONAL INTEREST\s+/i, 'Interest: ') // Clean up promotional interest
